...
public class LoaderBinary extends Loader{
    ...
    public LoaderIterator load() throws LockedException, NotReadyException, 
        ...
        if(!isValidFile()) throw new LoaderException();
        ...
        while(reader.readBoolean()){
            ...
            if(textureFileEndPos > file.length()) throw new LoaderException();
            
            //notify that texture data is available
            if(listener != null && listener instanceof LoaderListenerBinary){
                ...
                if(texFile != null){
                    ...
                    if(counter != texLength) throw new LoaderException();
                    ...
                    if(!valid) throw new LoaderException();
                }else{
    ...
    private class LoaderIteratorBinary implements LoaderIterator{
         ...
         * understood
         */
        public LoaderIteratorBinary(LoaderBinary loader) throws IOException,
                LoaderException{
            this.loader = loader;
            listener = null;
        }
        ...
        public DataChunk next() throws NotAvailableException, LoaderException, 
            ...
            if(chunkSize < 0) throw new LoaderException();
            ...
            if(chunkEndPos > fileLength) throw new LoaderException();
            ...
            if(coordsSizeInBytes < 0) throw new LoaderException();
            ...
            if(colorsSizeInBytes < 0) throw new LoaderException();
            ...
            if(indicesSizeInBytes < 0) throw new LoaderException();
            //if size in bytes is not multiple of float size (4 bytes), then
            //file is corrupted
            ...
            //corrupted
            if(texCoordsSizeInBytes < 0) throw new LoaderException();
            //if size in bytes is not multiple of float size (4 bytes), then
            //file is corrupted
            ...
            //corrupted
            if(normalsSizeInBytes < 0) throw new LoaderException();
            //if size in bytes is not multiple of float size (4 bytes), then
            //file is corrupted
