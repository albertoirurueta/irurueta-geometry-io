...
public class LoaderSTL extends Loader{
    ...
    private class LoaderIteratorSTL implements LoaderIterator{
        ...
        public DataChunk next() throws NotAvailableException, LoaderException, 
            ...
            try{
                if(isAscii){
                    ...
                    do{
                        ...
                        if(word.equalsIgnoreCase(ASCII_FACET)){
                            ...
                            if(word.equalsIgnoreCase(ASCII_NORMAL)){
                                ...
                                word = readNonEmptyWord();
                                nX = Float.valueOf(word).floatValue();
                                word = readNonEmptyWord();
                                nY = Float.valueOf(word).floatValue();
                                word = readNonEmptyWord();
                                nZ = Float.valueOf(word).floatValue();                                
                            }else throw new LoaderException(); //unexpected word
                            ...
                            //read vertex data
                            word = readNonEmptyWord();
                            coordX = Float.valueOf(word).floatValue();
                            word = readNonEmptyWord();
                            coordY = Float.valueOf(word).floatValue();
                            word = readNonEmptyWord();
                            coordZ = Float.valueOf(word).floatValue();
                    ...
                    }while(!endOfFileReached && word != null);
                
                    //check if end of file was reached before reaching end of data
                    if(!endOfFileReached && !endOfChunk) 
                        throw new LoaderException();
                }else{
                    boolean progressNeedsToBeComputed = false;
                    
                    do{
                        //read vertex data (triangle normal, triiangle vertices
                    ...
                    //to ensure progress is computed if all 3 vertices of a 
                    //triangle cannot fit in this chunk
                    if(progressNeedsToBeComputed){
                        //compute progress
                        if(loader.listener != null){
                            if((reader.getPosition() % progressStep) == 0){
                                loader.listener.onLoadProgressChange(loader, 
                                    (float)(reader.getPosition()) / 
                                    (float)(fileLength));
                            }
        ...
        private void setUp() throws IOException, LoaderException{
            ...
            if(n != magicStartLength) throw new LoaderException();
            String str = new String(buffer);
            
            isAscii = str.equalsIgnoreCase(ASCII_START);
            if(isAscii){
                //check 
                if(reader.isEndOfStream()) throw new LoaderException(); 
                //load solid name
